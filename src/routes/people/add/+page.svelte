	...
	{#if form?.message}
		<div class="flex flex-col items-center justify-center gap-2">
			...
			<button class="bg-blue-500 px-2 py-1 text-white hover:bg-blue-700"
				><a href="/people">View People</a></button
			>
		</div>
	{:else}
		<form use:enhance method="POST">
			...
			<input
				...
				required
			/>
			<label for="MiddleInitial" class="block text-sm font-medium text-gray-700"
			...
			<input
				...
				id="MiddleInitial"
				maxlength="1"
				class="w-10 rounded-none border border-black px-2 outline-none focus:border-indigo-600"
			/>
